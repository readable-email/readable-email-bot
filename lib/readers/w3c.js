...
function get(url, attempts) {
  ...
  return request(url).then(function (res) {
    ...
    console.dir(contentType && contentType[1]);
    if (contentType && contentType[1] && contentType[1] === 'windows-1252') {
      return win1252.decode(res.getBody().toString('binary'));
    }
    return res.getBody().toString(contentType ? contentType[1] : 'utf8');
...
readMessage('http://lists.w3.org/Archives/Public/www-style/2014Aug/0370.html').done(console.dir.bind(console));
