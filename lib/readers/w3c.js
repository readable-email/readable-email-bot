...
function get(url, attempts) {
  ...
  return request(url).then(function (res) {
    ...
    var contentType = /<meta +http-equiv=\"Content-Type\" +content=\"text\/html; charset=([^\"]+)\" *\/\>/i.exec(res.getBody('utf8'));
    var encoding = (contentType ? contentType[1] : 'utf8').trim().toLowerCase();
    try {
      return res.getBody().toString(encoding);
    } catch (ex) {
      return legacy.decode(res.getBody(), encoding);
    }
  }).then(null, function (err) {
    if (attempts < 2 || err instanceof TypeError) throw err;
...
function readMessage(url) {
  return get(url).then(function (body) {
    ...
    body.replace(/\<span id\=\"from\"\>\n\<dfn\>From\<\/dfn\>: (.*)\&lt\;\<a href\=\"[^\"]*\">(.*)\<\/a\>\&gt\;\n\<\/span\>/g, function (_, name, email) {
      ...
      from = {
        name: ent.decode(name).trim(),
        email: ent.decode(email).trim()
      };
    });
    ...
    return {
      ...
      subject: subject,
      from: from,
      date: utils.handleMileniumBug(new Date(date)),
      body: messageBody
    };
  }).then(null, function (err) {
    if (err instanceof Error) err.message += ' (' + JSON.stringify(url) + ')';
    throw err;
  });
}
