...
function get(url, attempts) {
  ...
  return request(url).then(function (res) {
    ...
    var contentType = /<meta +http-equiv=\"Content-Type\" +content=\"text\/html; charset=([^\"]+)\" *\/\>/.exec(res.getBody('utf8'));
    var encoding = (contentType ? contentType[1] : 'utf8').trim().toLowerCase();
    if (encoding === 'iso-8859-1'|| encoding === 'windows-1252') {
      return win1252.decode(res.getBody().toString('binary'));
    }
    if (encoding === 'koi8-r') {
      return koi8r.decode(res.getBody().toString('binary'));
    }
    if (encoding === 'us-ascii') encoding = 'utf8';
